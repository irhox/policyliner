...
public class DeltaPresenceService implements PrivacyMetricService<DeltaPresenceReport> {
    ...
    public List<DeltaPresenceReport> computeMetricForTables(List<String> tableNames, JsonQuasiIdentifiers localQuasiIdentifiers) {
        ...
        for (JsonQuasiIdentifier identifier : quasiIdentifiers.getQuasiIdentifiers()) {
            if (tableNames.contains(identifier.getViewName())) {
                try {
                    DeltaPresenceReport report = calculator.getPrivacyMetricReportOfTable(
    ...
    public void evaluatePolicyAgainstMetric(Policy policy, JsonQuasiIdentifiers localQuasiIdentifiers) {
        ...
        PrivacyMetric lowerDeltaPresence = policyPrivacyMetrics.stream().
                filter(p -> p.name.equals(PrivacyMetricName.DELTA_PRESENCE.getValue()) && p.metricSeverity.equals(MetricSeverity.LOWER_LIMIT))
                .findFirst().orElse(null);
        PrivacyMetric upperDeltaPresence = policyPrivacyMetrics.stream()
                .filter(p -> p.name.equals(PrivacyMetricName.DELTA_PRESENCE.getValue()) && p.metricSeverity.equals(MetricSeverity.UPPER_LIMIT))
                .findFirst().orElse(null);
        PrivacyMetric middleDeltaPresence = policyPrivacyMetrics.stream()
                .filter(p -> p.name.equals(PrivacyMetricName.DELTA_PRESENCE.getValue()) && p.metricSeverity.equals(MetricSeverity.MIDDLE_VALUE))
                .findFirst().orElse(null);
